...
/datum/weather/effect/wind_gust/proc/apply_gust_effect(mob/living/L, direction)
	...
	playsound(L, pick(wind_sounds), rand(30, 50))
	...
	for(var/obj/nearby_object in range(location, 1))
		if(!nearby_object || nearby_object.anchored || HAS_TRAIT(nearby_object, TRAIT_NODROP))
			continue

		if(nearby_object.w_class < WEIGHT_CLASS_NORMAL)
		...
		else if(nearby_object.w_class < WEIGHT_CLASS_GIGANTIC)
			if(prob(25 * severity))
...
/datum/weather/effect/lightning_strike/proc/strike_turf(turf/T)
	...
	for(var/mob/living/L in T.contents)
		...
		to_chat(L, span_warning("You feel an intense shock as lightning courses through you!"))
		L.emote("scream")
		L.Stun(4, TRUE)
		L.blur_eyes(3)
		addtimer(CALLBACK(proc=L.set_bluriness, args=list(0)), 3)
